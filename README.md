...
A starter for audit-style actions, failure handling, and simple unlock flows that make security code easier to reason about.

## Highlights
- audit and failure simulation commands
- stateful examples for lock and unlock flows
- diagnostics that show when security-relevant paths were used
...
Built jars are written to `build/libs/hytale-plugin-template-security-1.1.0.jar`, with matching sources and javadoc jars next to it.

## Commands
- `/hdsecuritydemo`: Runs a demo action for the Security template.
- `/hdsecuritystatus`: Shows runtime status for the Security template.
- Common actions: `info, toggle, sample, audit-demo, fail-demo, unlock-demo`
...
2. Copy `build/libs/hytale-plugin-template-security-1.1.0.jar` into your server `plugins/` directory.
